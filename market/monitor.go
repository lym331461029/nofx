...
type WSMonitor struct {
	...
	klineDataMap6h sync.Map // 存储每个交易对的K线历史数据
...
var subKlineTime = []string{"1h", "4h", "6h"}
...
func (m *WSMonitor) initializeHistoricalData() error {
	...
	for _, symbol := range m.symbols {
		...
		go func(s string) {
			...
			// 获取历史K线数据
			klines6h, err := apiClient.GetKlines(s, "6h", 200)
			if err != nil {
				log.Printf("获取 %s 历史数据失败: %v", s, err)
				return
			}
			if len(klines6h) > 0 {
				m.klineDataMap6h.Store(s, klines6h)
				log.Printf("已加载 %s 的历史K线数据-6h: %d 条", s, len(klines6h))
			}
		}(symbol)
	}
...
func (m *WSMonitor) GetCurrentKlines(symbol string, _time string) ([]Kline, error) {
	...
	if !exists {
		...
		apiClient := NewAPIClient()
		klines, err := apiClient.GetKlines(symbol, _time, 200)
